...
import { englishWords, explain, toPhoneme } from "../src/index.ts";
...
import { applySchwa, schwaSource } from "../src/schwa.ts";
...
describe("KBBI-verified plain e", () => {
  ...
  test.each([
    // Read by a native speaker on 2026-09-08, where no KBBI lafal exists.
    ["sekjen", "seʔdʒen"],
    ["bekel", "bəkəl"],
    ["bendel", "bəndəl"],
    ["geber", "gəber"],
    ["woles", "woləs"],
    ["are", "are"],
  ])("reads %s as the speaker does", (word, expected) => {
    expect(toPhoneme(word).phonemes).toBe(expected);
  });
...
/**
 * Corrections and additions from Wiktionary's marked headwords, generated by
 * scripts/dump-wiktionary.py into src/data/schwa-wiktionary.ts. One word per
 * shape of change, so a refresh that silently drops the layer fails here.
 */
describe("Wiktionary corrections", () => {
  test.each([
    ["energi", "energi"],
    ["label", "label"],
    ["tempo", "tempo"],
    ["celeng", "tʃeleŋ"],
    ["geber", "gəber"],
  ])("reads %s as Wiktionary marks it", (word, expected) => {
    expect(toPhoneme(word).phonemes).toBe(expected);
  });

  test("names the layer in explain", () => {
    expect(explain("energi")[0]?.source).toBe("wiktionary");
  });

  test("places a word no other table lists", () => {
    // The affix rules read `denger` as `deŋer`; Wiktionary marks dêngêr.
    expect(explain("denger")[0]).toEqual({
      word: "denger",
      phonemes: "dəŋər",
      source: "wiktionary",
    });
  ...
  test("a hand-written override still beats it", () => {
    // Both list `cetak` with the same mask; the override is the one consulted last.
    expect(explain("cetak")[0]?.source).toBe("override");
  });

  test("leaves a part-of-speech homograph to its rule", () => {
    // Wiktionary marks only the noun `pèr`; the common preposition is /pər/.
    expect(toPhoneme("per").phonemes).toBe("pər");
  });
});

/**
 * The English table is generated with every Indonesian-listed word removed,
 * so English is only ever consulted last. A new Indonesian table, or a
 * refresh of one, must be followed by `scripts/dump-english.py`, or the two
 * disagree and the Indonesian reading is shadowed for nothing.
 */
describe("the English table yields to every Indonesian table", () => {
  test("no English word is also placed by an Indonesian source", () => {
    const shadowed = englishWords().filter(
      (word) => schwaSource(word) !== "rules" && schwaSource(word) !== "affix"
    );
