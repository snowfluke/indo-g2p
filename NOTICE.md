...
`src/data/schwa-wiktionary.ts` lists the words whose marked Wiktionary
headword contradicts the schwa dictionary, and the words no other table has,
reduced to a bitmask per word. It is generated by `scripts/dump-wiktionary.py`
from the kaikki.org extract of the Indonesian Wiktionary
(<https://kaikki.org/dictionary/Indonesian/>), which is not vendored.
